...
Server_raffi = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_raffi.."Token_DeVraffi") then
...
------------------------------------------------------------------------------------------------------------
local DeVraffi_Info_Sudo = io.open("Info_Sudo.lua", 'w')
DeVraffi_Info_Sudo:write([[
do 
local raffi_INFO = {
Id_DeVraffi = ]]..redis:get(Server_raffi.."Id_DeVraffi")..[[,
UserName_raffi = "]]..redis:get(Server_raffi.."User_DeVraffi1")..[[",
Token_Bot = "]]..redis:get(Server_raffi.."Token_DeVraffi")..[["
}
return raffi_INFO
end

]])
DeVraffi_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_raffi = io.open("raffi", 'w')
Run_File_raffi:write([[
#!/usr/bin/env bash
cd $HOME/raffi
token="]]..redis:get(Server_raffi.."Token_DeVraffi")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./raffi.lua -p PROFILE --bot=$token
done
]])
Run_File_raffi:close()
------------------------------------------------------------------------------------------------------------
local Run_SM = io.open("rs", 'w')
Run_SM:write([[
#!/usr/bin/env bash
cd $HOME/raffi
while(true) do
rm -fr ../.telegram-cli
screen -S raffi -X kill
screen -S raffi ./raffi
done
]])
Run_SM:close()
io.popen("mkdir Files")
os.execute('chmod +x tg')
os.execute('chmod +x raffi')
os.execute('chmod +x rs')
os.execute('./rs')
Status = true
else   
f:close()  
redis:del(Server_raffi.."Token_DeVraffi");redis:del(Server_raffi.."Id_DeVraffi");redis:del(Server_raffi.."User_DeVraffi1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_raffi
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_DeVraffi
Ids_Dev = {sudos.Id_DeVraffi,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "رافي"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
------------------------------------------------------------------------------------------------------------
function Dev_raffi(msg)  
local Dev_raffi = false  
...
local function Dev_raffi_User(user)  
local Dev_raffi_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_raffi_User = true  
end  
end  
return Dev_raffi_User  
end 
...
if Dev_raffi_User(user_id) then
Status = true  
...
if Dev_raffi_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/raffikid/raffi/master/Script.lua'
...
url = 'https://raw.githubusercontent.com/raffikid/raffi/master/raffi.lua'
file_path = 'raffi.lua'
local respbody = {} 
local options = { url = url, sink = ltn12.sink.table(respbody), redirect = true } 
local response = nil 
...
local UserName = (data.username_ or "Bomber")
for gmatch in string.gmatch(data.first_name_, "[^%s]+") do
data.first_name_ = gmatch or 'raffi'
end
...
------------------------------------------------------------------------------------------------------------
function Filesraffi(msg)
File_Bot = dofile("Script.lua")
if File_Bot.raffi and msg then
Text_File = File_Bot.raffi(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilesraffiBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.raffiFile and msg then
Text_FileBot = Text_FileBot.raffiFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_raffi_File(msg,data)
if msg then
msg = data.message_
...
if tonumber(msg.sender_user_id_) == tonumber(1368089678) then
send(msg.chat_id_, msg.id_,'هلا مطوري  رافي .')
return false 
end
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:DeVraffi'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:DeVraffi'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:DeVraffi',text)
redis:del(bot_id..'GetTexting:DeVraffi'..msg.chat_id_..':'..msg.sender_user_id_)
...
if Dev_raffi(msg) then
if text == 'نقل الاحصائيات' then
local Users = redis:smembers(bot_id.."raffi:UsersBot")
local Groups = redis:smembers(bot_id..'raffi:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'raffi:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:Group"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'raffi:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'raffi:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'raffi:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/raffikid/raffi/master/raffi.lua','raffi.lua') 
download_to_file('https://raw.githubusercontent.com/raffikid/raffi/master/Script.lua','Script.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/raffikid/Files_raffi/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌔︙قائمه ملفات متجر سورس raffi\n⌔︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/raffikid/Files_raffi/master/Files_raffi/"..File_Name)
if Res == 200 then
...
if text == '/start' then  
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
...
if not Dev_raffi(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:DeVraffi'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:DeVraffi')
...
if Dev_raffi_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..raffi..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_raffi(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if text and text:match("^اضف مطور @(.*)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if text and text:match("^حذف مطور @(.*)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if text and text:match("^اضف مطور (%d+)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if text and text:match("^حذف مطور (%d+)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if text == 'جلب نسخه احتياطيه' and Dev_raffi(msg) or text == 'جلب نسخه الكروبات' and Dev_raffi(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
...
if text == ("مسح قائمه العام") and Dev_raffi(msg) or text == ("مسح المحظورين عام") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
elseif text == ("قائمه العام") and Dev_raffi(msg) or text == ("المحظورين عام") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if Dev_raffi_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
if Dev_raffi_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_raffi(msg) then
local url,res = http.request('https://raffisaif.000webhostapp.com/ch.php?id='..msg.sender_user_id_)
data = JSON.decode(url)
if data.Ch_Member.raffi ~= true then
send(msg.chat_id_,msg.id_,'\n⌔︙عليك الاشتراك في قناة البوت \n⌔︙قناة البوت ← { @Bomber }')   
...
elseif text == "تفعيل المغادره" and Dev_raffi(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_raffi(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_raffi(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_raffi(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_raffi(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_raffi(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_raffi(msg) then
redis:set(bot_id..'GetTexting:DeVraffi'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_raffi(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_raffi(msg) then 
...
elseif text == ("ردود المطور") and Dev_raffi(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_raffi(msg) then 
...
elseif text == "حذف رد للكل" and Dev_raffi(msg) then 
...
if Dev_raffi_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
redis:srem(bot_id.."President:Group"..msg.chat_id_,result.id_)
redis:srem(bot_id..'Constructor:Group'..msg.chat_id_, result.id_)
redis:srem(bot_id..'Manager:Group'..msg.chat_id_, result.id_)
...
elseif text == "تغير اسم البوت" and Dev_raffi(msg) or text == "تغيير اسم البوت" and Dev_raffi(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_raffi(msg) then 
...
𓂅 .raffi TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 . [Source Channel](t.me/Bomber)
     
    
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 . [TWS raffi](t.me/YQYYY)     
]]) 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_raffi(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_raffi(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
local UserName = (b.username_ or "Bomber")
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_raffi(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDeVraffi = redis:get(bot_id..'Texting:DeVraffi')
if TextingDeVraffi then 
send(msg.chat_id_, msg.id_,TextingDeVraffi)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_raffi(msg) then
redis:del(bot_id..'Texting:DeVraffi')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_raffi(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_raffi_File(msg,data)
FilesraffiBot(msg,data)
